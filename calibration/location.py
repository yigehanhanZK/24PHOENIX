...
import sys
sys.path.append('..')
from cameraClass.camera import CameraThread
from cameraClass.common import read_yaml
...
def __callback_1(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        # 周围200*200像素放大图
        rect = cv2.getWindowImageRect(param["pick_win_name"])
        ...
        cv2.imshow(param["zoom_win_name"], img_cut)
        cv2.moveWindow(param["zoom_win_name"], rect[0] - 400, rect[1] + 200)
        cv2.resizeWindow(param["zoom_win_name"], 400, 400)
...
def locate_pick(cap: CameraThread, enemy, camera_type, home_size=False, video_test=False):
    ...
    info["pick_win_name"] = "pick_corner"
    info["zoom_win_name"] = "zoom_in"
    info["pick_flag"] = False
    info["pick_point"] = None  # 回调函数中点击的点

    cv2.namedWindow(info["pick_win_name"], cv2.WINDOW_NORMAL)
    cv2.resizeWindow(info["pick_win_name"], 1280, 780)
    cv2.setWindowProperty(info["pick_win_name"], cv2.WND_PROP_TOPMOST, 1)
    cv2.moveWindow(info["pick_win_name"], 500, 300)
    ...
    while True:
        ...
        cv2.imshow(info["pick_win_name"], info["pick_img"])
